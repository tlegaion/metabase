        ...
        (doseq [field-or-expr [[:field (mt/id :attempts :datetime) {}]
          ...
          (testing (format "last %s" unit)
            (let [query (mt/mbql-query attempts
              ...
              (is (= (assoc-in query [:query :filter]
                               [:and
                                [:>=
                   ...
                   (optimize-temporal-filters query)))))
          (testing (format "this %s" unit)
            ...
            (doseq [clause [[:relative-datetime 0]
              ...
              (testing (format "clause = %s" (pr-str clause))
                (let [query (mt/mbql-query attempts
                              {:aggregation [[:count]]
                               :aggregation-idents {0 "BYKIJ19tzaL37DgJKXlWa"}
                               :expression  {"date" $datetime}
                               :expression-idents {"date" "Ps9PYMasV7C_yYaiqoAw0"}
                               :filter      [:=
                                             (mbql.u/assoc-field-options field-or-expr :temporal-unit unit)
                                             clause]})]
                  (is (= (assoc-in query [:query :filter]
    ...
    (qp.store/with-metadata-provider meta/metadata-provider
      (let [query (lib.tu.macros/mbql-query users
        ...
        (is (= (assoc-in query [:query :filter]
                         [:and
                          [:>=
                           [:+ [:field (meta/id :users :last-login) {:temporal-unit :default}] [:interval 3 :month]]
                           [:relative-datetime -12 :month]]
                          [:<
                           [:+ [:field (meta/id :users :last-login) {:temporal-unit :default}] [:interval 3 :month]]
                           [:relative-datetime 1 :month]]])
