...
(mr/def ::AggregationIdents
  "Aggregations get their `:ident` in legacy MBQL from a separate map, which maps the index of the aggregation
  to its ident.
...
(mr/def ::MBQLQuery
  [:and
   [:map
    ...
    [:aggregation-idents {:optional true} [:ref ::AggregationIdents]]
  ...
  primarily used to let power things like binning when used with Field Literals instead of normal Fields."}
   ...
   #_[:fn
    ...
    (fn [{:keys [expressions expression-idents]}]
      (prn expressions expression-idents)
      (or (= nil expressions expression-idents)
          (and (map? expressions)
